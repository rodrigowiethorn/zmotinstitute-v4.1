<template>
    <!-- This example requires Tailwind CSS v2.0+ -->
<div class="relative bg-indigo-700 m-6 lg:m-12 rounded-3xl" id="ZmotAuditFeatures">

  
  <div class="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:py-12 lg:px-8 lg:grid lg:grid-cols-3 lg:gap-x-8">
    <div>
      <!-- <h2 class="text-base font-semibold text-indigo-600 uppercase tracking-wide">Everything you need</h2> -->
      <p class="mt-2 text-3xl text-center md:text-left font-extrabold text-white">Palestrantes Que Vão Encantar Sua Platéia
      </p>
      <p class="mt-8 lg:mr-8 text-lg text-justify md:text-left text-gray-200">Se você está organizando um evento e precisa de um tema super atual e relevante, com um storytelling envolvedor, nossas Palestras sobre ZMOT e Comportamento do Consumidor são o que você precisa.</p>
        
      <!-- <p class="mt-8 lg:mr-8 text-lg text-justify md:text-left text-gray-200">Seja em um evento presencial ou online, os participantes do seu evento vão te agradecer por ajudá-los a entender como o ZMOT pode melhorar os resultados de forma exponencial.</p> -->
    </div>
    <div class="mt-8 lg:mt-0 lg:col-span-2">
      <dl class="space-y-10 sm:space-y-0 sm:grid sm:grid-cols-2 sm:grid-rows-3 sm:grid-flow-col sm:gap-x-6 sm:gap-y-10 lg:gap-x-8">
        <div class="flex">
          <!-- Heroicon name: check -->
          <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div class="ml-3">
            <dt class="text-lg leading-6 font-medium text-white">
              Análise Jornada de Compra
            </dt>
            <dd class="mt-2 text-base text-indigo-200">
              You can manage phone, email and chat conversations all from a single mailbox.
            </dd>
          </div>
        </div>

        <div class="flex">
          <!-- Heroicon name: check -->
          <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div class="ml-3">
            <dt class="text-lg leading-6 font-medium text-white">
              Definição Personas
            </dt>
            <dd class="mt-2 text-base text-indigo-200">
              You can manage phone, email and chat conversations all from a single mailbox.
            </dd>
          </div>
        </div>

        <div class="flex">
          <!-- Heroicon name: check -->
          <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div class="ml-3">
            <dt class="text-lg leading-6 font-medium text-white">
              Análise Jornada de Compra
            </dt>
            <dd class="mt-2 text-base text-indigo-200">
              You can manage phone, email and chat conversations all from a single mailbox.
            </dd>
          </div>
        </div>

        <div class="flex">
          <!-- Heroicon name: check -->
          <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div class="ml-3">
            <dt class="text-lg leading-6 font-medium text-white">
              Mídia Paga (PPC)
            </dt>
            <dd class="mt-2 text-base text-indigo-200">
              Find what you need with advanced filters, bulk actions, and quick views.
            </dd>
          </div>
        </div>

        <div class="flex">
          <!-- Heroicon name: check -->
          <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div class="ml-3">
            <dt class="text-lg leading-6 font-medium text-white">
              Boards
            </dt>
            <dd class="mt-2 text-base text-indigo-200">
              Find what you need with advanced filters, bulk actions, and quick views.
            </dd>
          </div>
        </div>

        <div class="flex">
          <!-- Heroicon name: check -->
          <svg class="flex-shrink-0 h-6 w-6 text-green-500" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
          </svg>
          <div class="ml-3">
            <dt class="text-lg leading-6 font-medium text-white">
              Reporting
            </dt>
            <dd class="mt-2 text-base text-indigo-200">
              Find what you need with advanced filters, bulk actions, and quick views.
            </dd>
          </div>
        </div>

      </dl>
    </div>
  </div>
</div>
</template>

<script>
export default {
  name: "SectionTalksLecturesFeaturesList"
}
</script>